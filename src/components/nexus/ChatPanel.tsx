...
import { ArrowUp, BellRing, FileText, Loader2 } from "lucide-react";
...
function Markdown({ children }: { children: string }) {
  return (
    <div className="space-y-2 text-sm leading-relaxed [&_a]:text-primary [&_code]:rounded [&_code]:bg-surface-2 [&_code]:px-1 [&_li]:ml-4 [&_li]:list-disc [&_strong]:text-foreground [&_strong]:font-semibold">
...
function Bubble({ message, isLatest }: { message: ChatMessage; isLatest: boolean }) {
    ...
    return (
      <div className="msg-rise max-w-[92%]">
        <div className="rounded-xl border border-[color-mix(in_oklab,var(--agent-sentinel)_45%,transparent)] border-l-4 border-l-[var(--agent-sentinel)] bg-[color-mix(in_oklab,var(--agent-sentinel)_9%,var(--surface))] px-4 py-3">
          <div className="mb-1.5 flex items-center gap-1.5 text-[11px] font-semibold tracking-wide text-[var(--agent-sentinel)] uppercase">
            <BellRing className="size-3.5" /> NEXUS noticed something
          </div>
  ...
  return (
    <div className="msg-rise max-w-[92%]">
      <div
        className="mb-1.5 inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-semibold tracking-wide uppercase"
        style={{
          ...
          <div className="mt-2.5 flex flex-wrap gap-1.5">
            {message.citations.map((c, i) => (
              <button
                ...
                className="inline-flex items-center gap-1 rounded-full border border-border bg-surface-2 px-2 py-0.5 text-[10px] text-muted-foreground transition-colors hover:text-foreground"
              >
                <FileText className="size-3" /> Source: {c.title} {c.section}
              </button>
            ))}
          </div>
        )}
      ...
      <div className="mt-1 text-[10px] text-muted-foreground">{timeOf(message.created_at)}</div>
    </div>
  );
}
      ...
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5">
        {messages.length === 0 && !running && (
          <div className="flex h-full flex-col items-center justify-center gap-3 text-center">
            <div className="rounded-2xl border border-border bg-surface px-6 py-5">
              <p className="text-sm font-semibold">Ask NEXUS anything about your campus life</p>
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {[
                ...
                <button
                  key={s}
                  onClick={() => onSend(s)}
                  className="max-w-[240px] truncate rounded-full border border-border bg-surface-2 px-3 py-1.5 text-[11px] text-muted-foreground transition-colors hover:border-primary hover:text-foreground"
                >
                  {s}
                </button>
              ))}
        ...
        {running && (
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <Loader2 className="size-3.5 animate-spin" style={{ color: thinkingMeta?.hue }} />
            <span>
      ...
      <div className="border-t border-border p-3">
        <div className="flex items-end gap-2 rounded-xl border border-border bg-surface-2 p-2 focus-within:border-primary">
          <Textarea
            ...
            className="max-h-32 min-h-[38px] resize-none border-0 bg-transparent text-sm shadow-none focus-visible:ring-0"
          />
